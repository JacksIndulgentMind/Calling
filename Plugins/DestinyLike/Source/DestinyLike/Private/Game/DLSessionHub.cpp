...
void UDLSessionHub::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
	int32 PortIni = Port;
	GConfig->GetInt(TEXT("/Script/DestinyLike.DLLobbySettings"), TEXT("SessionHubPort"), PortIni, GGameIni);
	Port = FMath::Clamp(PortIni, 1024, 65535);
}

void UDLSessionHub::Deinitialize()
{
	StopHost();
	Super::Deinitialize();
}

TStatId UDLSessionHub::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(UDLSessionHub, STATGROUP_Tickables);
}

bool UDLSessionHub::IsTickable() const
{
	return !IsTemplate() && ListenSocket != nullptr;
}

void UDLSessionHub::StartHost()
{
	...
	ListenSocket = FTcpSocketBuilder(TEXT("DLSessionHub"))
		.AsReusable()
	...
	UE_LOG(LogDestinyLike, Display, TEXT("DestinyLike: session hub ws://0.0.0.0:%d"), Port);
#endif
}

void UDLSessionHub::StopHost()
{
...
void UDLSessionHub::Tick(float DeltaTime)
{
	(void)DeltaTime;
	if (!ListenSocket)
	{
...
void UDLSessionHub::AcceptClients()
{
	if (!ListenSocket)
	{
		return;
	...
	FSocket* Client = ListenSocket->Accept(TEXT("DLSessionHubClient"));
	if (!Client)
	{
		return;
	}
...
void UDLSessionHub::CloseClient(int32 Index)
{
...
bool UDLSessionHub::ReadHttpHeader(const TArray<uint8>& Buffer, FString& OutKey)
{
...
FString UDLSessionHub::MakeAcceptKey(const FString& ClientKey)
{
...
bool UDLSessionHub::TryUpgrade(int32 Index)
{
...
bool UDLSessionHub::DecodeFrame(TArray<uint8>& Buffer, FString& OutText)
{
	OutText.Reset();
	if (Buffer.Num() < 2)
	{
...
void UDLSessionHub::SendText(int32 Index, const FString& Text)
{
...
void UDLSessionHub::HandleText(int32 Index, const FString& Text)
{
	...
	Type = Type.ToLower();
	if ((Type == TEXT("join") || Type == TEXT("subscribe")) && Out->GetBoolField(TEXT("ok")))
	{
		FString SeatStr;
		if (Out->TryGetStringField(TEXT("seatId"), SeatStr))
		{
			FGuid::Parse(SeatStr, Clients[Index].SeatId);
		}
	}
	FString Json;
...
void UDLSessionHub::PushSnapshots(EDLHubSnapshotReason Reason, const FGuid& OnlySeat)
{
	UDLLobbySubsystem* Lobby = GetGameInstance() ? GetGameInstance()->GetSubsystem<UDLLobbySubsystem>() : nullptr;
	UDLAgentBridgeSubsystem* Bridge = GetGameInstance() ? GetGameInstance()->GetSubsystem<UDLAgentBridgeSubsystem>() : nullptr;
	...
	switch (Reason)
	{
	case EDLHubSnapshotReason::LobbyDirty: ReasonName = TEXT("lobbyDirty"); break;
	case EDLHubSnapshotReason::LowLookahead: ReasonName = TEXT("lowLookahead"); break;
	default: break;
	}
	...
	for (int32 i = 0; i < Clients.Num(); ++i)
	{
		...
		const UDLParticipantSeat* Seat = Lobby->FindSeat(Clients[i].SeatId);
...
void UDLSessionHub::PumpClient(int32 Index)
{
	FClient& Client = Clients[Index];
	if (!Client.Socket)
	{
