...
ADLGreyboxFloors::ADLGreyboxFloors()
{
...
void ADLGreyboxFloors::PostInitializeComponents()
{
	Super::PostInitializeComponents();
	EnsureBuilt();
}

void ADLGreyboxFloors::BeginPlay()
{
	Super::BeginPlay();
	EnsureBuilt();
	// Recast needs a frame after the cubes register, and nav must run for every
	// layout — EnsureBuilt used to skip it when geometry was already built.
	NavRebuildDelay = 0.15f;
	if (Sky)
	{
		Sky->RecaptureSky();
	}
...
void ADLGreyboxFloors::Tick(float DeltaSeconds)
{
	Super::Tick(DeltaSeconds);
	if (NavRebuildDelay >= 0.f)
	{
		NavRebuildDelay -= DeltaSeconds;
		if (NavRebuildDelay <= 0.f)
		{
			NavRebuildDelay = -1.f;
			RebuildNavigation();
...
void ADLGreyboxFloors::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(ADLGreyboxFloors, Layout);
}

ADLGreyboxFloors* ADLGreyboxFloors::SpawnIfMissing(UWorld* World, EDLGreyboxLayout Layout)
{
	if (!World)
	{
		return nullptr;
	...
	ADLGreyboxFloors::ApplyVoidWorldSettings(World);

	// Placed map cubes (SocialPad, or a leftover PvP floor) sit at one Z and hide the ravine.
	const bool bHideAllPlacedFloors = (Layout == EDLGreyboxLayout::PvpThreeLane);
	...
	for (TActorIterator<ADLGreyboxFloors> It(World); It; ++It)
	{
		ADLGreyboxFloors* Existing = *It;
		Existing->Layout = Layout;
		Existing->EnsureBuilt();
		if (Layout == EDLGreyboxLayout::PvpThreeLane)
		{
	...
	ADLGreyboxFloors* Floors = World->SpawnActorDeferred<ADLGreyboxFloors>(
		ADLGreyboxFloors::StaticClass(), FTransform::Identity, nullptr, nullptr,
...
APlayerStart* ADLGreyboxFloors::EnsurePlayerStart(UWorld* World, const FVector& Location)
{
	if (!World)
	{
		return nullptr;
...
APlayerStart* ADLGreyboxFloors::EnsureTaggedPlayerStart(UWorld* World, FName Tag, const FVector& Location, const FRotator& Rotation)
{
	if (!World)
	{
		return nullptr;
...
void ADLGreyboxFloors::ApplyVoidWorldSettings(UWorld* World)
{
	if (!World)
	{
		return;
	...
	{
		WorldSettings->KillZ = -50000.f;
		WorldSettings->bEnableWorldBoundsChecks = false;
	}
}

void ADLGreyboxFloors::RescueFallenPawns() const
{
	UWorld* World = GetWorld();
	ADLGameModeBase* GM = World ? World->GetAuthGameMode<ADLGameModeBase>() : nullptr;
	if (!GM)
	...
	for (FConstPlayerControllerIterator It = World->GetPlayerControllerIterator(); It; ++It)
	{
		...
		if (Pawn && Pawn->GetActorLocation().Z < GetRescueMinZ())
		{
			GM->RequestRespawn(PC);
		}
	}
...
FVector ADLGreyboxFloors::GetPlayerStartLocation() const
{
	switch (Layout)
	{
	case EDLGreyboxLayout::SocialLeviathan:
	{
		...
		return FVector(GSocialLeviathanPlayerStart.X, GSocialLeviathanPlayerStart.Y, GSocialLeviathanPlayerStart.Z + 50.f);
	}
	case EDLGreyboxLayout::SocialSquare:
		return FVector(0.f, 0.f, 130.f);
	case EDLGreyboxLayout::PvpBannerfall:
		return FVector(-5000.f, 0.f, 130.f);
	case EDLGreyboxLayout::PvpThreeLane:
		return FVector(-14500.f, 0.f, 130.f);
	case EDLGreyboxLayout::RaidShuro:
		return FVector(-3500.f, 0.f, 130.f);
	case EDLGreyboxLayout::RaidVault:
		return FVector(0.f, 0.f, 130.f);
	default:
...
FVector ADLGreyboxFloors::GetBluePlayerStartLocation() const
{
	if (Layout == EDLGreyboxLayout::PvpThreeLane)
	{
		return FVector(14500.f, 0.f, 130.f);
...
float ADLGreyboxFloors::GetRescueMinZ() const
{
	if (Layout == EDLGreyboxLayout::PvpThreeLane)
	{
		// Pit floor is -20 m; never yank someone standing in the ravine.
		return -2300.f;
	}
	return GetPlayerStartLocation().Z - 150.f;
}

float ADLGreyboxFloors::GetSuggestedArenaHalfExtent() const
{
	switch (Layout)
	{
	case EDLGreyboxLayout::RaidKalli:
		return 2200.f;
	case EDLGreyboxLayout::RaidShuro:
		return 600.f;
	case EDLGreyboxLayout::RaidMorgeth:
		return 1600.f;
	case EDLGreyboxLayout::RaidVault:
		return 700.f;
	default:
...
void ADLGreyboxFloors::EnsureBuilt()
{
	// PvP is rebuilt every EnsureBuilt so a cached flat slab cannot survive a code change.
	if (bHasBuilt && BuiltLayout == Layout && Platforms.Num() > 0
		&& Layout != EDLGreyboxLayout::PvpThreeLane)
	{
		return;
	}
...
void ADLGreyboxFloors::AddPlatform(const FVector& CenterCm, float SizeXMeters, float SizeYMeters, float SizeZCm)
{
	AddBox(
...
void ADLGreyboxFloors::AddBox(const FVector& CenterCm, const FVector& SizeCm, const FRotator& Rotation)
{
...
void ADLGreyboxFloors::StampModule(FName Id, const FVector& CenterCm, const FRotator& Rotation)
{
...
void ADLGreyboxFloors::StampFillFloor(const FVector& CenterCm, float SizeXMeters, float SizeYMeters, float SlabZCm)
{
...
void ADLGreyboxFloors::BuildPvpThreeLane()
{
	FDLPvpThreeLaneRecipe R;
	R.Load();
	const float M = 100.f;
	...
	UE_LOG(LogDestinyLike, Display,
		TEXT("Greybox PvpThreeLane padZ=0 courtZ=%.0f courtM=%.0f laneW=%.0f rampPitch=%.1f deg red=%s"),
		PitZ, CourtM, LaneW, PitchDeg,
		*GetPlayerStartLocation().ToCompactString());
}

void ADLGreyboxFloors::BuildLayout()
{
	if (TUniquePtr<IDLGreyboxLayout> Built = DLMakeGreyboxLayout(Layout))
	{
		Built->Build(*this);
	}
}

void ADLGreyboxFloors::ApplyVisibleShading()
{
...
void ADLGreyboxFloors::RebuildNavigation()
{
	UWorld* World = GetWorld();
	...
	if (!NavSys)
	{
		UE_LOG(LogDestinyLike, Warning, TEXT("Greybox: no navigation system after AddNavigationSystemToWorld"));
		return;
	}
	...
	{
		if (It->ActorHasTag(FName(TEXT("DLGreyboxNavBounds"))))
		{
			Vol = *It;
			break;
		}
	...
	if (!Vol)
	{
		...
		if (Vol)
		{
			Vol->Tags.Add(FName(TEXT("DLGreyboxNavBounds")));
		}
	}
	if (Vol)
	{
	...
	if (Recast)
	{
		...
		DLNavLinkPolicy::ApplyToRecast(*Recast, SurvivingDropCm);
	}
	...
	UE_LOG(LogDestinyLike, Display, TEXT("Greybox nav rebuilt geo=%s nav=%s tiles=%d pads=%d jumpLinks=%s"),
		*Box.ToString(),
		*NavBox.ToString(),
