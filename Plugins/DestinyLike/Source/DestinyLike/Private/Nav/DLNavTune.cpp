...
namespace
{
	FString ConfigPath()
	{
		...
		{
			return FPaths::Combine(Plugin->GetBaseDir(), TEXT("Config/Nav/NavTune.json"));
		}
		return FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("DestinyLike/Config/Nav/NavTune.json"));
	}

	void AddDefaultLinks(FDLNavTune& Tune)
	{
		auto Link = [](const TCHAR* Name, float Len, float Edge, const TCHAR* Depth, const TCHAR* Height, const TCHAR* Ends,
			float Filter, const TCHAR* Down, const TCHAR* Up) -> FDLNavLinkTune
		{
			FDLNavLinkTune L;
			L.Name = FName(Name);
			L.JumpLength = Len;
	...
	FDLNavTune LoadOrDefault()
	{
		...
		{
			UE_LOG(LogDestinyLike, Warning, TEXT("DestinyLike: missing NavTune.json; using compiled defaults"));
			return Tune;
		}
		...
		{
			UE_LOG(LogDestinyLike, Warning, TEXT("DestinyLike: NavTune.json failed to parse; using compiled defaults"));
			return Tune;
		}
...
const FDLNavTune& DLNavTune::Get()
{
	static const FDLNavTune Cached = LoadOrDefault();
	return Cached;
}

float DLNavTune::ResolveScalar(const FString& TokenOrNumber, float Fallback, const FDLNavTune& Tune, float SurvivingDropCm)
{
