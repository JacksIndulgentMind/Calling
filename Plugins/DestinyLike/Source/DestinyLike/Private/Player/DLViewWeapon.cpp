...
namespace
{
		...
		const FWeaponSee& See)
	{
		...
		DLViewWeapon::ShowFamily(Root, false);
		return Root;
	}
}

USceneComponent* DLViewWeapon::BuildOnCamera(USceneComponent* Camera)
{
...
USceneComponent* DLViewWeapon::BuildOnBody(USceneComponent* Body)
{
...
void DLViewWeapon::UpdateAdsPose(USceneComponent* WeaponRoot, float AdsAlpha, FName SightId, float KickPitch)
{
	if (!WeaponRoot)
	{
		return;
	...
	FVector Ads = AdsIron;
	if (SightId == FName(TEXT("scope")))
	{
		Ads = AdsScope;
	}
	else if (SightId != FName(TEXT("iron")))
	{
		Ads = AdsRedDot;
	}
...
void DLViewWeapon::ShowFamily(USceneComponent* WeaponRoot, bool bGrenadeRifle)
{
	if (!WeaponRoot)
	{
		return;
...
void DLViewWeapon::ShowSight(USceneComponent* WeaponRoot, FName SightId)
{
	if (!WeaponRoot)
	{
		return;
	}
	const bool bIron = SightId == FName(TEXT("iron"));
	const bool bScope = SightId == FName(TEXT("scope"));
...
void DLViewWeapon::SetThirdPersonPeek(USceneComponent* ViewRoot, USceneComponent* WorldRoot, bool bPeek,
...
USceneComponent* DLViewWeapon::FindMuzzle(USceneComponent* WeaponRoot)
{
...
USceneComponent* DLViewWeapon::FindEjector(USceneComponent* WeaponRoot)
{
