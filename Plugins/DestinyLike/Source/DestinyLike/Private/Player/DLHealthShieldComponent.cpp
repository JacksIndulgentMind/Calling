...
UDLHealthShieldComponent::UDLHealthShieldComponent()
{
	PrimaryComponentTick.bCanEverTick = true;
	SetIsReplicatedByDefault(true);
}

void UDLHealthShieldComponent::BeginPlay()
{
	Super::BeginPlay();
	ReloadSettings();
...
void UDLHealthShieldComponent::ReloadSettings()
{
	Tune.LoadFromIni();
}

void UDLHealthShieldComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(UDLHealthShieldComponent, Health);
	DOREPLIFETIME(UDLHealthShieldComponent, Shield);
	DOREPLIFETIME(UDLHealthShieldComponent, bAlive);
}

float UDLHealthShieldComponent::ApplyDamage(float Damage, AController* InstigatorController, bool bPrecision)
{
...
void UDLHealthShieldComponent::ApplyFlinch(float Strength)
{
	float Posture = 1.f;
	if (const AActor* Owner = GetOwner())
	{
		if (const UDLCombatMovementComponent* Move = Owner->FindComponentByClass<UDLCombatMovementComponent>())
		{
			if (Move->IsSliding())
			{
				Posture = 0.45f;
...
FVector2D UDLHealthShieldComponent::ConsumeHipKick()
{
...
void UDLHealthShieldComponent::Heal(float Amount)
{
	if (!bAlive) return;
...
void UDLHealthShieldComponent::ResetToFull()
{
	Health = Tune.MaxHealth;
	Shield = Tune.MaxShield;
	bAlive = true;
...
void UDLHealthShieldComponent::RestoreShield(float Amount)
{
	if (!bAlive) return;
...
void UDLHealthShieldComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
