...
TSharedRef<SWidget> UDLBootProfileWidget::RebuildWidget()
{
	BuildWidgetTree();
	return Super::RebuildWidget();
}

void UDLBootProfileWidget::BuildWidgetTree()
{
	...
	TitleText = MakeLabel(WidgetTree, TEXT("TitleText"), TEXT("Create Guardian"), 28);
	...
	ExistingProfileCombo->OnSelectionChanged.AddDynamic(this, &UDLBootProfileWidget::HandleExistingProfileChanged);
	...
	ContinueButton = WidgetTree->ConstructWidget<UButton>(UButton::StaticClass(), TEXT("ContinueButton"));
	{
		...
		ContinueButton->OnClicked.AddDynamic(this, &UDLBootProfileWidget::HandleContinueClicked);
	}
	...
	ProfileNameBox->SetHintText(FText::FromString(TEXT("Guardian")));
	...
	CreateEnterButton = WidgetTree->ConstructWidget<UButton>(UButton::StaticClass(), TEXT("CreateEnterButton"));
	{
		...
		CreateEnterButton->OnClicked.AddDynamic(this, &UDLBootProfileWidget::HandleCreateAndEnterClicked);
	}
...
void UDLBootProfileWidget::PopulateClassCombo()
{
...
void UDLBootProfileWidget::PopulateSexCombo()
{
	if (!SexCombo) return;
...
void UDLBootProfileWidget::NativeConstruct()
{
...
void UDLBootProfileWidget::RefreshExistingProfiles()
{
	...
	UDLProfileSubsystem* Profiles = GetGameInstance() ? GetGameInstance()->GetSubsystem<UDLProfileSubsystem>() : nullptr;
	if (!Profiles)
	{
	...
	for (const FDLLocalProfile& P : Profiles->GetAllProfiles())
	{
...
void UDLBootProfileWidget::HandleExistingProfileChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
	(void)SelectedItem;
	(void)SelectionType;
}

void UDLBootProfileWidget::SetStatus(const FString& Message, bool bError)
{
...
void UDLBootProfileWidget::EnterSocial()
{
	if (UGameInstance* GI = GetGameInstance())
	{
		if (UDLSceneRouter* Router = GI->GetSubsystem<UDLSceneRouter>())
		{
...
void UDLBootProfileWidget::HandleCreateAndEnterClicked()
{
...
void UDLBootProfileWidget::HandleContinueClicked()
{
...
bool UDLBootProfileWidget::ContinueExistingAndEnter()
{
	UDLProfileSubsystem* Profiles = GetGameInstance() ? GetGameInstance()->GetSubsystem<UDLProfileSubsystem>() : nullptr;
...
bool UDLBootProfileWidget::CommitNewProfileAndEnter()
{
	UDLProfileSubsystem* Profiles = GetGameInstance() ? GetGameInstance()->GetSubsystem<UDLProfileSubsystem>() : nullptr;
	if (!Profiles)
	{
	...
	if (ClassCombo)
	{
		...
		if (ClassStr.Equals(TEXT("Pathfinder"), ESearchCase::IgnoreCase)) ClassId = EDLClassId::Pathfinder;
		else if (ClassStr.Equals(TEXT("Warden"), ESearchCase::IgnoreCase)) ClassId = EDLClassId::Warden;
	}
	...
	if (SexCombo)
	{
		...
		if (SexStr.Equals(TEXT("Male"), ESearchCase::IgnoreCase)) Sex = EDLCharacterSex::Male;
		else if (SexStr.Equals(TEXT("Female"), ESearchCase::IgnoreCase)) Sex = EDLCharacterSex::Female;
	}

	FDLLocalProfile Profile = Profiles->CreateProfile(ProfileName);
