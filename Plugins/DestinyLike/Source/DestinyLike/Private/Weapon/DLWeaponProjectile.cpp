...
ADLWeaponProjectile::ADLWeaponProjectile()
{
...
void ADLWeaponProjectile::IgnoreInstigator()
{
...
void ADLWeaponProjectile::InitTracer(APawn* InInstigator, const FVector& Direction, float Speed, float LifeSeconds)
{
...
void ADLWeaponProjectile::InitCasing(APawn* InInstigator, const FVector& Impulse)
{
	InstigatorPawn = InInstigator;
	bGrenade = false;
	bCasing = true;
...
void ADLWeaponProjectile::InitGrenade(APawn* InInstigator, const FVector& Direction, float Speed, float LifeSeconds,
...
void ADLWeaponProjectile::Detonate()
{
	if (bDetonated)
	{
		return;
	...
	{
		if (ADLAbilityAoE* AoE = World->SpawnActor<ADLAbilityAoE>(GetActorLocation(), FRotator::ZeroRotator))
		{
			AoE->InitAoE(InstigatorPawn.Get(), ExplodeRadius, 0.28f, Damage, 0.f);
		}
		else
		{
			DLAbilityCombat::ApplyDamageInRadius(World, InstigatorPawn.Get(), GetActorLocation(), ExplodeRadius, Damage);
		}
	}
	Destroy();
}

bool ADLWeaponProjectile::CheckProximity() const
{
	...
	for (TActorIterator<AActor> It(GetWorld()); It; ++It)
	{
		...
		if (!Other->FindComponentByClass<UDLDamageableComponent>()
			&& !Other->FindComponentByClass<UDLHealthShieldComponent>())
		{
			continue;
		}
...
void ADLWeaponProjectile::Tick(float DeltaSeconds)
{
	...
	if (bCasing)
	{
		...
		FCollisionQueryParams Params(SCENE_QUERY_STAT(DLCasingFly), false, this);
		if (APawn* Inst = InstigatorPawn.Get())
		{
	...
	FHitResult Hit;
	const FVector Start = GetActorLocation();
	FCollisionQueryParams Params(SCENE_QUERY_STAT(DLGrenadeFly), false, this);
	if (APawn* Inst = InstigatorPawn.Get())
	{
		Params.AddIgnoredActor(Inst);
	}
