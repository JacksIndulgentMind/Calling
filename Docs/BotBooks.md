...
`GET /state?seat=` includes `botBook: { name, nodeId, verb, whiles, leafElapsed, pawnStill, focusSeat, enemyDistXY, enemyX, enemyY, stack, stackLen, remaining, remainingLen, queued, queueLen, jit, lastBranchCause, lastBranchNodeId, lastBranchBook, executionFails, executionError, followAlert, lastFollowAlert, followed }`. `followAlert` is **this tick** (`botbook_unknown_verb`, `botbook_unknown_while`, `botbook_missing_marker`, `botbook_not_local`, `botbook_goto_start_failed`, `botbook_goto_stick_clobber`, `botbook_goto_no_stick`, `botbook_goto_loc_still`, `botbook_idle`). `lastFollowAlert` is the latched code after the condition clears. `followed` is false only while `followAlert` is live. `loc_still` is a **living** goto: pawn loc not moving ~80 cm for 0.5 s **and** velocity Size ≤ 80, with goal DistXY still outside the leaf’s outer settle (GoodEnough distXY if set, else Success distXY, else 300) — not Recast waypoint `gotoStuck`. A dead pawn resets the still clock and does **not** fire `loc_still` — dump `/state.lastHit` / `lastShot` / `lastDeath` (`valid`, `kind`, `source`, `killer`, `killerName`, `amount`, remaining health/shield, victim and killer loc, t) and pawn `hit` / `shot` / `death`; `events` `code=death` `detail=kind|source|killerName`. `lastShot` is the last instigated gun/ability hit and is not overwritten by status DoT. Airborne travel (jump/drop) resets the still clock; a grounded perch does not. One **execution** hit (`unknown_*` / `not_local` / `goto_start_failed` / `stick_clobber` / `missing_marker`) sets `executionError`. The **first** followAlert or `cause=execution` also **fails the PvP match** (`modeResult=fail`, `modeFailReason` is that code) and appends `/state.events`. Pollers dump `events` and stop — do not resume `in_progress`. A pass is `modeResult=winner` with `botbook_append` / `kill` / `shrine_held` / `mode_winner` in `events`. Raid/debug stroll fail-fasts on `lastDeath.valid` (a melt is not Recast). `/state` also has `agentMoveX` / `agentMoveY` / `agentFire` / `lookTrack` / `lookTrackSeat`. Helper: `Scripts/dl-assert-command-followed.ps1`.
...
`goto` on a catalog file **must** use `marker=id`. xyz on a catalog file is a load error. `marker=live_occupy` (aliases `live_shrine` / `liveShrine`) resolves to `/state.liveOccupy` when the leaf starts. `marker=live_orbit` is the current occupy pad’s clockwise orbit crumbs `{occupyId}_orbit_0`…`_N`: first pick after arrive is the crumb **farthest from the focus pawn** (flank); each later `goto live_orbit` advances clockwise and wraps. Success/GoodEnough must be tighter than the adjacent crumb chord (pad-center is not already Success). Do not invent an `orbit` verb — keep Recast `goto`.
