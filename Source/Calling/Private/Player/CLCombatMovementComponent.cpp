...
UCLCombatMovementComponent::UCLCombatMovementComponent()
{
...
void UCLCombatMovementComponent::BeginPlay()
{
	Super::BeginPlay();
	ReloadSettings();
...
void UCLCombatMovementComponent::SetMoveInput(FVector2D MoveXY)
{
	PendingMove = MoveXY;
}

void UCLCombatMovementComponent::SetWantsSprint(bool bSprint) { bWantsSprint = bSprint; }
void UCLCombatMovementComponent::SetWantsCrouch(bool bCrouch) { bWantsCrouch = bCrouch; }
void UCLCombatMovementComponent::RequestSlide() { bSlideRequested = true; }
void UCLCombatMovementComponent::SetIsADS(bool bADS) { bIsADS = bADS; }

float UCLCombatMovementComponent::GetFullSprintSpeed() const
{
	return Tune.BaseWalkSpeed * (1.f + MobilityBonus) * Tune.SprintSpeedMultiplier;
}

FVector UCLCombatMovementComponent::GetInputMoveWorldDir() const
{
...
FVector UCLCombatMovementComponent::GetPendingMoveWorldDir() const
{
...
void UCLCombatMovementComponent::SetDodgeInvulnerable(bool bInvuln)
{
	if (AActor* Owner = GetOwner())
	{
		if (UCLHealthShieldComponent* HS = Owner->FindComponentByClass<UCLHealthShieldComponent>())
		{
...
void UCLCombatMovementComponent::NotifyJumped()
{
	if (bDiving)
	{
		return;
	...
	if (IsMovingOnGround())
	{
		...
		if (JumpStyle == ECLJumpStyle::InertiaDamp)
		{
	...
	{
		if (JumpStyle == ECLJumpStyle::RocketPulse)
		{
			--JumpsRemaining;
...
float UCLCombatMovementComponent::EstimateSlideTravelCm() const
{
...
bool UCLCombatMovementComponent::HasFloorAt(const FVector& Loc) const
{
	...
	FHitResult Hit;
	FCollisionQueryParams Params(SCENE_QUERY_STAT(CLSlideFloor), false, Owner);
...
bool UCLCombatMovementComponent::CanCommitSlideInDir(const FVector& Dir) const
{
...
float UCLCombatMovementComponent::GetDodgeAlpha() const
{
...
float UCLCombatMovementComponent::GetDiveFlipDegrees() const
{
	if (!bDiving)
	{
		return 0.f;
...
float UCLCombatMovementComponent::GetKneeLandAlpha() const
{
	return FMath::Clamp(KneeLandRemaining / 0.35f, 0.f, 1.f);
}

void UCLCombatMovementComponent::EndSlide()
{
	if (!bSliding)
	{
		return;
...
void UCLCombatMovementComponent::BeginSlide()
{
	bSliding = true;
	SlideElapsed = 0.f;
...
float UCLCombatMovementComponent::SlideSpeedAtAlpha(float Alpha) const
{
...
bool UCLCombatMovementComponent::TryEnterSlideFromTick()
{
...
bool UCLCombatMovementComponent::TryStartSlide()
{
	RequestSlide();
	return TryEnterSlideFromTick();
}

bool UCLCombatMovementComponent::TryDash(FVector Direction, float DistanceOverride)
{
	if (bSliding)
	{
...
bool UCLCombatMovementComponent::TryDodge()
{
	if (bSliding)
	{
...
bool UCLCombatMovementComponent::TryAirDive()
{
	if (IsMovingOnGround())
	{
		return false;
...
bool UCLCombatMovementComponent::TryMantle()
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(CLMantle), false, GetOwner());
...
void UCLCombatMovementComponent::UpdateSpeeds()
{
...
void UCLCombatMovementComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	if (IsMovingOnGround())
	{
		JumpsRemaining = Tune.MaxJumps;
	...
	else if (!bDiving && (JumpStyle != ECLJumpStyle::InertiaDamp || HoverRemaining <= 0.f))
	{
		GravityScale = 1.f;
	}
...
void UCLCombatMovementComponent::UpdateCrouchTransition(float DeltaTime)
{
	...
	if (const ACLPlayerCharacter* CLChar = Cast<ACLPlayerCharacter>(Char))
	{
		if (UStaticMeshComponent* Body = CLChar->GetBodyMesh())
		{
