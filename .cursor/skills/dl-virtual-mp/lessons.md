...
- Host first. Guest HTTP `18765` / hub `18766` bind fail is expected. Director `virtualhost` / `ready` / `go` only on the process that owns `/state`.
...
- `ACLGreyboxFloors` replicates. Any `DOREPLIFETIME` property needs `Replicated` or `ReplicatedUsing` or the **guest fatals** on join. `Layout` uses `OnRep_Layout` so the client stamps pads locally (runtime cubes are not the net channel).
...
- Default `/state` is the host pawn. Two agents: `/state?seat=` per cursor seat after `mindControl`. Never one probe for two bodies.
